...
var two_stage_bucket_flag = 0;
var drag_chute_emergency_deploy_flag = 0;
...
var full_ignition = func{
	...
	var initial_throttle = 33 / (getprop("/mission/launch/throttle-nominal-percent") - 67);

    ssme_array[0].ignite();
    ssme_array[1].ignite();
    ssme_array[2].ignite();

	#Throttle to 100 % initially until 60 fps (ap stage 1 transition)
    ssme_array[0].throttle_to(initial_throttle);
    ssme_array[1].throttle_to(initial_throttle);
    ssme_array[2].throttle_to(initial_throttle);
...
#No bucket if engine(s) is(are) failed (Bucket based on Vtrue)
if (engop == 3)
	{
	
	if ((vt > getprop("/fdm/jsbsim/systems/ap/launch/throttle-down-boundary-1")) and (two_stage_bucket_flag == 1) and (SRB_burn_timer > 0.0))
		{
		SpaceShuttle.callout.make("Atlantis, Go at first stage bucket throttle down!", "real");
		two_stage_bucket_flag = 2;
		}

	else if ((vt > getprop("/fdm/jsbsim/systems/ap/launch/throttle-down-boundary-2")) and (launch_message_flag == 0) and (SRB_burn_timer > 0.0))
		{
		#two stage bucket
		if (two_stage_bucket_flag == 2) {SpaceShuttle.callout.make("Atlantis, Go at second stage bucket throttle down!", "real");}
		#One stage bucket
		else {SpaceShuttle.callout.make("Atlantis, Go at throttle down!", "real");}
		
		launch_message_flag = 1;
		}

	else if ((vt > getprop("/mission/launch/throttle-up-velocity")) and (launch_message_flag ==1))
		{
...
# in case of an RTLS, we want to init PRTLS entry loop
if (getprop("/fdm/jsbsim/systems/entry_guidance/guidance-mode") == 3)
	{
	if (SpaceShuttle.EGD.RTLS_START_LOOP == 0) {SpaceShuttle.compute_TAEM_guidance_targets();}
	}
...
var update_rms_view = func {
	...
	var cam_eff_z = getprop("/fdm/jsbsim/systems/rms/camera-effector-x") + SpaceShuttle.rms.shoulder.x;
	var cam_eff_x = getprop("/fdm/jsbsim/systems/rms/camera-effector-y") + SpaceShuttle.rms.shoulder.y;
	var cam_eff_y = getprop("/fdm/jsbsim/systems/rms/camera-effector-z") + SpaceShuttle.rms.shoulder.z;
	...
	#roll with the payload but does not follow the end effector roll induced by arm yaw/pitch
	#	var cam_eff_roll = getprop("/fdm/jsbsim/systems/rms/ang-wrist-roll-deg");

	#always roll with the end effector, but suffer from Euler singularity at P approaching 90 degrees
	var cam_eff_roll = getprop("/fdm/jsbsim/systems/rms/camera-sum-wrist-roll-deg");

	
#	print ("shoulder= ", SpaceShuttle.rms.shoulder.x, " ", SpaceShuttle.rms.shoulder.y, " ",SpaceShuttle.rms.shoulder.z," ");
...
#Entry guidance for OPS 6 superseded by newer PRTLS guidance function
if (getprop("/fdm/jsbsim/systems/entry_guidance/guidance-mode") > 0)
	{
	if (SpaceShuttle.EGD.START_LOOP == 0) {SpaceShuttle.compute_TAEM_guidance_targets();}
	}
...
#DAP mode out of TRANSITION logic
if (SpaceShuttle.orbital_dap_manager.dap_mode != "NONE") {SpaceShuttle.orbital_dap_manager.load_dap("NONE");}
...
var d_remain = getprop("/fdm/jsbsim/systems/entry_guidance/remaining-distance-nm"); #Driven by TRANGE
...
if (ops == 6) #OPS 6 GRTLS and TAEM
	...
	#Update GRTLS guidance loop for distance to runway in mm 602 and 603 with GRTLS Guidance Formalism
	if (GRTLS.INIT_PASS == 0) {SpaceShuttle.compute_TAEM_guidance_targets();}
	...
	#TAEM GRTLS high energy logic to be added in normal TAEM for better handling of early S-turns

	#During alpha transition 
	if (((altitude < 60000.0) or (mach < 3.2) or (getprop("/fdm/jsbsim/systems/dps/major-mode") == 603)) and (deorbit_stage_flag > 1) and (TAEM_guidance_TGINIT.IPHASE == 4))
		{
		#setprop("/sim/messages/copilot", "TAEM interface reached.");

		#No automatic TAEM transition for Contigency with no ECAL (TAEM entered after 603 manually inserted)
		if ((GRTLS.PRTLS == "ON") or (GRTLS.ECAL == "ON") or ((GRTLS.CONT == "ON") and (getprop("/fdm/jsbsim/systems/dps/major-mode") == 603)))
			{
			#GRTLS init for OPS6 TAEM (after alpha transition)
			SpaceShuttle.TAEM_guidance_TGINIT.GRTLS_TAEM_INIT = 1;
			SpaceShuttle.callout.make("TAEM interface reached.", "info");
			setprop("/controls/shuttle/hud-mode",3);

			print(" GRTLS TAEM INIT FLAG is: ", TAEM_guidance_TGINIT.GRTLS_TAEM_INIT);
	...
	if (guidance_mode > 0)
		...
		if (((SpaceShuttle.EGRT_data.TRANGE < 50.0) or (mach < 2.5)) and (deorbit_stage_flag == 3))
			{
			SpaceShuttle.compute_TAEM_guidance_targets();
			glide_loop();
...
#One time computation for Entry remaining distance // to be updated with newer loop 
...
#Sb state
#0.005 for opening and 0.01 for closing (Closing rate is twice faster than opening rate) // 0.005 everywhere if on APU remains (PRL)
#SB loop speed in TAEM guidance (with a settimer at 0.10, it gives 10°/s for closing and 5 °/s for opening)

var increase_speedbrake = func {

var speedbrake_state = getprop("/fdm/jsbsim/systems/fcs/speedbrake-cmd-norm");
var opening_rate = 0.005;

#Entry update speed loop is 1s vs 0.1s for TAEM
if (SpaceShuttle.TAEM_guidance_available == 0) {opening_rate = opening_rate * 10;}

speedbrake_state = speedbrake_state + opening_rate;
...
var speedbrake_state = getprop("/fdm/jsbsim/systems/fcs/speedbrake-cmd-norm");
var closing_rate = 0.01;

#PRL test (Closing rate halved if one hyd system remains)
if (getprop("/fdm/jsbsim/systems/apu/number-systems-pressurized") < 2) {closing_rate = 0.005;}
else {closing_rate = 0.01;}

#print("closing rate is: ", closing_rate);

#Entry update speed loop is 1s 
if (SpaceShuttle.TAEM_guidance_available == 0) {closing_rate = closing_rate * 10;}


speedbrake_state = speedbrake_state - closing_rate;
...
var deploy_chute = func {
	...
	#If SSME nozzle are not correctly stowed ( -0.45 rad), drag chute should not be deployed unless needed (Emergency case as per flight rules)
	if ((drag_chute_emergency_deploy_flag == 0) and ((getprop("/fdm/jsbsim/propulsion/engine[0]/pitch-angle-rad-cmd") == 0.0) or (getprop("/fdm/jsbsim/propulsion/engine[1]/pitch-angle-rad-cmd") == 0.0) or (getprop("/fdm/jsbsim/propulsion/engine[2]/pitch-angle-rad-cmd") == 0.0))) 
		{
		SpaceShuttle.callout.make("Drag chute should not be deployed if SSME nozzles are not stowed correctly!", "help");
		SpaceShuttle.callout.make("You can press on Deploy button one more time if you want to use drag chute deployement emergency mode", "help");
		if (drag_chute_emergency_deploy_flag == 0) {drag_chute_emergency_deploy_flag = 1;}
		return;
		}
...
if (stage == 3) 
	...
	if (getprop("/sim/presets/TAEM-approach-course-deg") != nil)	
		...
		{
		#Slight offset to be divergent from runway track for correct tangency point tracking
		setprop("/orientation/heading-deg", heading + 10); 
		}
	...
	var place_dist = 75000.0; # 45 miles downrange
	...
	#Exact Presetted Coordinates (Logic to be finished)
	#setprop("/position/latitude-deg", getprop("/sim/presets/latitude-deg")); 
	#setprop("/position/longitude-deg", getprop("/sim/presets/longitude-deg")); 
...
if (stage == 4) 
	...
	#13Nm // 14kft QFE for a 19° path
	setprop("/position/altitude-ft", 14000 + place_alt_correct);
...
setlistener("/fdm/jsbsim/systems/rms/rms-powered", func () {SpaceShuttle.cctv_overlay.init();},0,0);
