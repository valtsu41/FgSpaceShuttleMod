...
var orbital_dap_manager = {
	...
	control_select: func (mode) {
		...
		if ((me.major_mode != 201) and (me.major_mode != 202))
			{
			if ((me.dap_mode == "TRANSITION") and ((mode == "LVLH") or (mode == "FREE")))
				{
				print("Control mode ", mode ," is only supported in OPS 2");
				return;
				}
			else if (me.dap_mode != "TRANSITION")
				{
				print("Control mode ", mode ," is only supported in OPS 2");
				return;
				}
		...
		if (mode == "INRTL")
			...
			#FREE mode forces all rotation Rates in Pulse
	...
	rot_rate_select: func (mode, axis) {
		...
		if ((me.major_mode != 201) and (me.major_mode != 202) and (me.dap_mode != "TRANSITION"))
		
			{
			print("Axis rate selection is only supported in OPS 2 and with Transition DAP");
			return;
			}
