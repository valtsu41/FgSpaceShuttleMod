...
#Midvalue function (same than clamp, but argument's order might be disregarded)
var MIDVAL = func (X1, X2, X3) {

return X1 + X2 + X3 - math.max(X1, X2, X3) - math.min(X1, X2, X3);
...
var square_norm = v[0] * v[0] + v[1] * v[1] + v[2] * v[2];

if (debug.isnan(square_norm) == 0) {return math.sqrt(v[0] * v[0] + v[1] * v[1] + v[2] * v[2]);}
else
	{
...
if (norm(v) != 0) {return scalar_product(1./norm(v), v);}
...
###########################################################################################
# general helper functions for 3x3 matrix computations (based on jsbsim Matrix33 formalism)
###########################################################################################

#More general nasal matrix code: https://github.com/FGUM/LA/blob/main/Matrix.nas

#Matrix m = [m11,m21,m31,m12,m22,m32,m13,m23,m33];

#Determinant (3x3)
var matrix_determinant = func (m) {

return m[0] * m[4] * m[8] + m[3] * m[7] * m[2] + m[6] * m[1] * m[5] - m[6] * m[4] * m[2] - m[3] * m[1] * m[8] - m[7] * m[5] * m[0]; 
}


#Inverse (3x3)
var matrix_inverse = func (m) {

var outmat = [0,0,0,0,0,0,0,0,0];

if (matrix_determinant(m) != 0) 
	{
	var rdet = 1.0 / matrix_determinant(m);

	outmat[0] = rdet * (m[4] * m[8] - m[7] * m[5]);
    outmat[1] = rdet * (m[7] * m[2] - m[1] * m[8]);
    outmat[2] = rdet * (m[1] * m[5] - m[4] * m[2]);
    outmat[3] = rdet * (m[6] * m[5] - m[3] * m[8]);
    outmat[4] = rdet * (m[0] * m[8] - m[6] * m[2]);
    outmat[5] = rdet * (m[3] * m[2] - m[0] * m[5]);
    outmat[6] = rdet * (m[3] * m[7] - m[6] * m[4]);
    outmat[7] = rdet * (m[6] * m[1] - m[0] * m[7]);
    outmat[8] = rdet * (m[0] * m[4] - m[3] * m[1]);
	}

return outmat;
}

#Matrix Product with a 3D vector ( m is 3x3 and v is 3x1) / frame changes
var matrix_vector_product = func (m, v) {

var outvec = [0,0,0];

outvec[0] = v[0] * m[0] + v[1] * m[3] + v[2] * m[6];
outvec[1] = v[0] * m[1] + v[1] * m[4] + v[2] * m[7];
outvec[2] = v[0] * m[2] + v[1] * m[5] + v[2] * m[8];

return outvec;
}

#Matrix 3x3 Product 
var matrix_3d_product = func (m, n) {

var outvec = [0,0,0,0,0,0,0,0,0];

outvec[0] = m[0] * n[0] + m[3] * n[1] + m[6] * n[2];
outvec[3] = m[0] * n[3] + m[3] * n[4] + m[6] * n[5];
outvec[6] = m[0] * n[6] + m[3] * n[7] + m[6] * n[8];
outvec[1] = m[1] * n[0] + m[4] * n[1] + m[7] * n[2];
outvec[4] = m[1] * n[3] + m[4] * n[4] + m[7] * n[5];
outvec[7] = m[1] * n[6] + m[4] * n[7] + m[7] * n[8];
outvec[2] = m[2] * n[0] + m[5] * n[1] + m[8] * n[2];
outvec[5] = m[2] * n[3] + m[5] * n[4] + m[8] * n[5];
outvec[8] = m[2] * n[6] + m[5] * n[7] + m[8] * n[8];
...
#Geodetic to ECEF frame converter
var ECEF_converter = func (lat, lon, radius) {

var outvec = [0,0,0];
#var mean_radius = 20898950 + height; #feet (6370kmish)

outvec[0] = radius * math.cos(lat * 0.0174533) * math.cos(lon * 0.0174533);
outvec[1] = radius * math.cos(lat * 0.0174533) * math.sin(lon * 0.0174533);
outvec[2] = radius * math.sin(lat * 0.0174533);
...
var delta_time =  getprop("/fdm/jsbsim/systems/timer/delta-MET");

if (delta_time == 0) {return 0.0;} # we are on the pad

return getprop("/sim/time/elapsed-sec") + delta_time;
}
...
#Jsbsim ECEF to ECI vector at a given time (Epoch is at sim initialisation) / TIME == 0 correspond to current time
var M_CONV_EARTH_FIXED_TO_ECI = func(TIME) {

#Conversion matrix
var TIME_bis = getprop("/sim/time/elapsed-sec") + TIME;
var LAM = PEG.omega  * TIME_bis;
var M_ECEF_TO_ECI = [math.cos(LAM), math.sin(LAM), 0, -math.sin(LAM), math.cos(LAM), 0, 0, 0, 1];

#print("Time for matrix conv is : ", TIME);

return M_ECEF_TO_ECI;
};
