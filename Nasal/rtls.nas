...
#PRTLS flag for GRTLS loop and abort flaf for PEG guidance wip
if (GRTLS.PRTLS == "OFF") 
	{
	GRTLS.PRTLS = "ON";
	PEG.abort_flag = "RTLS";
...
if (powered_pitchdown_active == 1)
	...
	if (getprop("/fdm/jsbsim/systems/ap/rtls/powered-pitchdown-active") == 1)
		...
		#MECO conditions met or manual MECO
		if ((math.abs (-2.0 - alpha) < 1.0) or (manual_meco == 0))
			{
...
#Alpha recovery init
setprop("/fdm/jsbsim/systems/ap/grtls/alpha-recovery-active", 1);
...
if ((getprop("/fdm/jsbsim/accelerations/Nz") > 1.0) and (getprop("/fdm/jsbsim/systems/ap/grtls/Nz-tgt") == 0))
	{
		...
		#RTLS Nz hold max 2.2 Gs
		#if (getprop("/fdm/jsbsim/systems/abort/abort-mode") == 1) {Nz_target = math.min(2.2, Nz_target);}
