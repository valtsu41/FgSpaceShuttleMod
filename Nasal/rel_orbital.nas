...
var hst_manager_old = {
	...
	init : func (prox_x, prox_y, prox_z, dvx, dvy, dvz)  {
		...
		var model_path = "Aircraft/SpaceShuttle/Models/PayloadBay/HST/hst.xml";
...
#proximity computations in this module [see compute_proximity()] are
#done with respect to an inertial target, this is the default when
#targets are not within the proximity zone. The following conditionals
#may call for SpaceShuttle.proximity_manager.target_xyz and
#target_vxvyvz to be overloaded by the payload_manager / ISS manager's
#proximity computation (these ones are including the accurate tidal
#effects). When this happens, we don't want the default proximity
#computations done in this module to overload the buffers. For this,
#we use the following lock
		me.delegated_proximity = 0;
		...
		#print("rel_orbital: delegated_proximity= ",me.delegated_proximity);
		me.compute_proximity();
...
#initializing current_payload_reference to point to first target but
#only if not already associated to another target
			if (me.current_payload_reference == -1)
			{
			    me.current_payload_reference = 0;
			}
...
#if the target pointer is not associated, we completely skip any cycle
	        if (me.current_payload_reference == -1)
		{
		    return;
		}
#if not, we point to the next, unless we reach the last. In that case
#we move back to the first loaded and keep the target pointer
#initialized
		me.current_payload_reference += 1;
	...
	compute_proximity: func {
		...
		if ( me.delegated_proximity == 0 )
		{
		    ...
		    #print("rel_orbital proximity manager is computing proximity: ");
		    #me.list_proximity();

		}
		else
		{
		    #print("rel_orbital proximity manager is ON HOLD!");
		}      
	...
	compute_vdiff : func {
		...
		#let's not use prox_vy_alt.
		#
		#There is a lot of numerical jitter in the way vy_alt
		#is computed. Therefore, an artificially large vy
		#value may be passed to the payload manager once
		#proximity is detected. This sets the payload on a
		#very fast vy course till it exits the proximity
		#zone. Then, we land again here, and we loop till a
		#descent value of vy_alt finally allows the
		#payload_manager to definitely take over.
		##
		##me.target_prox_vy = -me.target_prox_vy_alt;		
		##
		##The way prox_vy is computed is more robust and behave sanely
