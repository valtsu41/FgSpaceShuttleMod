...
# Thorsten Renk 2018
	...
	if (airport == "KTTS")
		...
		settimer (func {SpaceShuttle.TAEM_TGEXEC(); }, 1.0);
	...
	else if (airport == "VQPR")
		{
		SpaceShuttle.update_site_by_index(91);
		if (abs(heading - 330.0) < 20.0) 
			{SpaceShuttle.update_runway_by_flag(0);}
		else
			{SpaceShuttle.update_runway_by_flag(1);}
		SpaceShuttle.TAEM_loop_running = 1;
		SpaceShuttle.compute_entry_guidance_target();
		settimer (func {SpaceShuttle.TAEM_TGEXEC(); }, 1.0);
		print("Automatic approach guidance for VQPR loaded");
		}
	else if (airport == "LOWI")
		{
		SpaceShuttle.update_site_by_index(92);
		if (abs(heading - 080.0) < 20.0) 
			{SpaceShuttle.update_runway_by_flag(0);}
		else
			{SpaceShuttle.update_runway_by_flag(1);}
		SpaceShuttle.TAEM_loop_running = 1;
		SpaceShuttle.compute_entry_guidance_target();
		settimer (func {SpaceShuttle.TAEM_TGEXEC(); }, 1.0);
		print("Automatic approach guidance for LOWI loaded");
		}
