...
if (major_function == 4) # BFS
	{
	SpaceShuttle.cws_last_message_acknowledge_bfs = 0;
	setprop("/fdm/jsbsim/systems/dps/bfs-error-string", "");
	setprop("/fdm/jsbsim/systems/cws/sm-alert-on", 0);
	setprop("/fdm/jsbsim/systems/cws/backup-alarm-on", 0); # Backup C/W is extinguished with MSG reset like SM alert ( trigger for Back up C/W switch)
	}
else # PASS
	{
	...
	setprop("/fdm/jsbsim/systems/cws/sm-alert-on", 0);
	setprop("/fdm/jsbsim/systems/cws/backup-alarm-on", 0);
	}
...
if (major_function == 4) # BFS
	...
	if (SpaceShuttle.cws_last_message_acknowledge_bfs == 1)
			...
			{
			var num_messages = size(SpaceShuttle.cws_message_array_bfs);
			if (num_messages > 1)
				{
				setprop("/fdm/jsbsim/systems/dps/bfs-error-string", SpaceShuttle.cws_message_array_bfs[num_messages - 2]);
				setsize(SpaceShuttle.cws_message_array_bfs, num_messages - 1);
				}
			}
...
if ((disp > 0) and (spec > 0)) 
	...
	if (spec == 2)
		...
		{
		page_select(idp_index, "p_dps_bearing");
		}
	else if (spec == 63)
		{
...
var spec54 = [301, 302, 303, 304, 602, 603]; #Entry / ECAL only for now (Jan 2025)
#var spec54 = [101, 102, 103, 104, 105, 106, 301, 302, 303, 304, 601, 602, 603];
var spec63 = [201, 202];
...
#Blank Oms_burn_logic peg7 hash variables 

SpaceShuttle.oms_burn_target.dvx_plan = 0;
SpaceShuttle.oms_burn_target.dvy_plan = 0;
SpaceShuttle.oms_burn_target.dvz_plan = 0;
}
...
#Blank Oms_burn_logic peg 4 hash variables 

SpaceShuttle.oms_burn_target.c1 = 0;
SpaceShuttle.oms_burn_target.c2 = 0;
SpaceShuttle.oms_burn_target.thetaT = 0;
SpaceShuttle.oms_burn_target.ht = 0;
SpaceShuttle.oms_burn_target.prplt = 0;
...
#"extDv" trigger when peg 7 entered via item 19,20,21 and not coming from Spec 34
SpaceShuttle.lambert_manager.lambert_peg7 = 0;
}
...
if ((header == "OPS") and (end =="PRO") and (major_function == 1))
	...
	if (major_mode == 101)
		...
		if (SpaceShuttle.nbat.checkout(2) == 0)
			{
			create_fault_message("S86 GPC       ", "CONF", "1234 ", 3, "PASS");  #S86 GPC    CONF   all PASS G
			}
		else
			{
		...
		if (SpaceShuttle.nbat.checkout(3) == 0)
			{
			create_fault_message("S86 GPC       ", "CONF", "1234 ", 3, "PASS");
			}
		else
			{
...
if ((header == "OPS") and (end =="PRO"))
	...
	if (major_mode == getprop("/fdm/jsbsim/systems/dps/major-mode"))
		...
		major_mode_transition(idp_index, "p_dps_mnvr");
		SpaceShuttle.mission_auto_OMS2();
		valid_flag = 1;
		}
	...
	if (major_mode == 301)
		...
		if ((SpaceShuttle.dps_simulation_detail_level == 1) and (SpaceShuttle.nbat.checkout(3) == 0))
			...
			if (getprop("/fdm/jsbsim/systems/abort/abort-mode") == 2)
				...
				setprop("/controls/shuttle/bf-mps-override",0);
				setprop("/controls/shuttle/control-system-string", "Aerojet");
				setprop("/sim/input/selected/engine[0]",1); # allow throttle to control SB
				major_mode_transition(idp_index, "p_entry");
				dk_listen_major_mode_transition(304);
				#SpaceShuttle.orbital_dap_manager.load_dap("NONE");
...
if ((header == "ITEM") and (end == "EXEC"))
	...
	if (((major_mode == 102) or (major_mode == 103) or (major_mode == 601)) and (spec == 0))
		...
		else if ((item == 5) and (GRTLS.CONT == "OFF"))
			{
	...
	if (((major_mode == 104) or (major_mode == 105) or (major_mode == 106) or (major_mode == 202) or (major_mode == 301) or (major_mode == 302) or (major_mode == 303)) and (spec == 0))
		...
		if (item == 1)
			{
			#Illegal Entry while burn is on
			if (oms_ignited == 1) {valid_flag = 0;}
			else
				{
				setprop("/fdm/jsbsim/systems/ap/oms-plan/burn-mode",1); 
				valid_flag = 1;

				#If guidance active, guidance is cancelled 
				if (burn_plan == 1) {SpaceShuttle.oms_guidance_reset();}
				}
			}
		else if (item == 2)
			{
			...
			if (oms_ignited == 1) {valid_flag = 0;}
				...
				{
				setprop("/fdm/jsbsim/systems/ap/oms-plan/burn-mode",2); 
				valid_flag = 1;

				#If guidance active, guidance is cancelled 
				if (burn_plan == 1) {SpaceShuttle.oms_guidance_reset();}

				#In case of previous OMS failure, failure indicator reseted if engine reselected
				if (SpaceShuttle.cws_msg_hash.omslpc == 1) {SpaceShuttle.cws_msg_hash.omslpc = 0;}
				}
			}
		else if (item == 3)
			{
			...
			if (oms_ignited == 1) {valid_flag = 0;}
				...
				{
				setprop("/fdm/jsbsim/systems/ap/oms-plan/burn-mode",3); 
				valid_flag = 1;
				...
				#In case of previous OMS failure, failure indicator reseted if engine reselected
				if (SpaceShuttle.cws_msg_hash.omsrpc == 1) {SpaceShuttle.cws_msg_hash.omsrpc = 0;}
				}
			}
		else if (item == 4)
			{
			...
			if (oms_ignited == 1) {valid_flag = 0;}
				...
				{
				setprop("/fdm/jsbsim/systems/ap/oms-plan/burn-mode",4); 
				valid_flag = 1;
		...
		else if ((item == 5) and (major_mode == 202))
			{
			#Tv roll always 180° in OPS2
			setprop("/fdm/jsbsim/systems/ap/oms-plan/tv-roll",num(value));
			valid_flag = 1;
			}
		else if (item == 6)
			{
			...
			if (oms_ignited == 1) {valid_flag = 0;}
				...
				{
				setprop("/fdm/jsbsim/systems/ap/oms-plan/trim-pitch",num(value)); 
				valid_flag = 1;
			...
			if (oms_ignited == 1) {valid_flag = 0;}
				...
				{
				setprop("/fdm/jsbsim/systems/ap/oms-plan/trim-yaw-left",num(value)); 
				valid_flag = 1;
			...
			if (oms_ignited == 1) {valid_flag = 0;}
				...
				{
				setprop("/fdm/jsbsim/systems/ap/oms-plan/trim-yaw-right",num(value)); 
				valid_flag = 1;
			...
			if (oms_ignited == 1) {valid_flag = 0;}
				...
				{
				setprop("/fdm/jsbsim/systems/ap/oms-plan/weight",num(value)); 
				valid_flag = 1;
			...
			if ((ops == 1) or (ops == 3))
				...
				SpaceShuttle.oms_burn_target.c1 = num(value);
				blank_peg7(); 
				valid_flag = 1;
				}
			}
			...
			if ((ops == 1) or (ops == 3))
				...
				setprop("/fdm/jsbsim/systems/ap/oms-plan/peg4-entered", 1); 
				SpaceShuttle.oms_burn_target.c2 = num(value);
				blank_peg7(); 
				valid_flag = 1;
				}
			}
			...
			if ((ops == 1) or (ops == 3))
				...
				setprop("/fdm/jsbsim/systems/ap/oms-plan/peg4-entered", 1); 
				SpaceShuttle.oms_burn_target.ht = num(value);
				blank_peg7(); 
				valid_flag = 1;
				}
			}
			...
			if ((ops == 1) or (ops == 3))
				...
				setprop("/fdm/jsbsim/systems/ap/oms-plan/peg4-entered", 1); 
				SpaceShuttle.oms_burn_target.thetaT = num(value);
				blank_peg7(); 
				valid_flag = 1;
				}
			}
			...
			if (ops == 3)
				...
				setprop("/fdm/jsbsim/systems/ap/oms-plan/peg4-entered", 1); 
				SpaceShuttle.oms_burn_target.prplt = num(value);
				blank_peg7(); 
				valid_flag = 1;
				}
			}

		#For Item 19 thru 21, if datas are entered while guidance is active, LOAD flashes again
		else if (item == 19)
			{
			#Illegal Entry while burn is on
			if (oms_ignited == 1) {valid_flag = 0;}
			...
			if ((burn_plan == 0) and (peg4_entered == 0))
				...
				{
				SpaceShuttle.oms_guidance_reset();
				}
			...
			#Reset of previous gimbal failure
			if (SpaceShuttle.cws_msg_hash.omslg == 1) {SpaceShuttle.cws_msg_hash.omslg = 0;}
			...
			#Reset of previous gimbal failure
			if (SpaceShuttle.cws_msg_hash.omsrg == 1) {SpaceShuttle.cws_msg_hash.omsrg = 0;}
	...
	if ((major_mode == 201) and (spec == 0))
		...
		if (item == 1)
			...
			{setprop("/fdm/jsbsim/systems/ap/ops201/mnvr-roll", -num(value)); valid_flag = 1;}   #Same sign for target inertial roll than the one display in the pfd and current roll in univ ptg (-jsb sim inertial roll)
		else if (item == 6)
	...
	if ((major_mode == 304) and (spec == 0))
		{
		if (item == 2) #Hdot bias reset
			{
			SpaceShuttle.EGD.DLRDOT = 0;
			valid_flag = 1;
			}
		...
		else if (item == 4) #Alt sites logic for Spec 54
			{
			if (SpaceShuttle.alt_sites.entry_flag == 0) {SpaceShuttle.alt_sites.entry_flag = 1;}
			else {SpaceShuttle.alt_sites.entry_flag = 0;}
			valid_flag = 1;
			}
		}

	#Vert situation Ecal toggle during Contigency Abort
	if ((major_mode == 602) and (spec == 0))
		{
		if ((item == 1) and (GRTLS.CONT == "ON"))
			{
			if (GRTLS.ECAL == "ON") {GRTLS.ECAL = "OFF";}
			else {GRTLS.ECAL = "ON";}
			valid_flag = 1;
			}
		}
		...
		if (item == 1)
			...
			if (SpaceShuttle.proximity_manager.iss_model == 0)
				...
				if (SpaceShuttle.lambert_manager.t1_in_progress == 0)
					...
					{
					SpaceShuttle.lambert_manager.interrupt();
					}
				}
			else
	...
	if (spec == 50)
		...
		if (item == 3)
			...
			if ((SpaceShuttle.TAEM_guidance_available == 1) or (major_mode == 304) or (GRTLS.INIT_PASS == 1))
				{
				SpaceShuttle.EGD.START = 0; #Reset of Entry Init
			...
			#It allows to have either an entry guidance computation or a TAEM one when we select the target in Spec50
			#Useful for scenarios when starting at Entry or TAEM interface
			if (major_mode == 304) 
				{
				#Reset of Entry Init
				SpaceShuttle.EGD.START = 0; 
				SpaceShuttle.compute_TAEM_guidance_targets();
				SpaceShuttle.compute_entry_guidance_target();
				}
			else if ((major_mode == 305) or (GRTLS.INIT_PASS == 1)) {SpaceShuttle.compute_TAEM_guidance_targets();}

			#TAL OPS1 Runway redesignation for PEG Vdmag computation
			else if ((major_mode == 103) and (PEG.abort_flag == "TAL"))
				{
				SpaceShuttle.compute_TAEM_guidance_targets();
				}
	...
	if (spec == 51)
		...
		if (item == 1)
			...
			{
			if ((major_mode == 102) or (major_mode == 103) or (major_mode == 104) or (major_mode == 105) or (major_mode == 106) or (major_mode == 601))
				{
				SpaceShuttle.toggle_property("/controls/shuttle/et-separation-auto-sw");
			
				if (SpaceShuttle.cws_msg_hash.et_sep_man == 1)
	...
	if (spec == 54)
		...
		if (item == 41)
			...
			if (value > 0)
				{
				SpaceShuttle.update_site_by_index(value); #Avoid "0" to be displayed at init
				SpaceShuttle.update_runway_by_flag(0); #Pri runway mandatory selection
				}
 
			#It allows to have either an entry guidance computation or a TAEM one when we select the target in Spec54
			#Useful for scenarios when starting at Entry or TAEM interface
			if (major_mode == 304) 
				{
				SpaceShuttle.compute_entry_guidance_target();
			...
			#else if (major_mode == 305) {SpaceShuttle.compute_TAEM_guidance_targets();}

			#CONT
			else if ((major_mode == 602) and (GRTLS.CONT == "ON")) {SpaceShuttle.compute_TAEM_guidance_targets();}
			valid_flag = 1;
			}
			...
			if (ops == 3) 
				{
				if ((value > 0) and (SpaceShuttle.alt_sites.entry_flag == 1))
					{
					var array = SpaceShuttle.set_alt_site_threshold(value);
					SpaceShuttle.landing_alt_site_one.set_latlon(array[0], array[1]);
					SpaceShuttle.alt_sites.one_index = array[2];
					SpaceShuttle.alt_sites.one_iata = array[3];
					SpaceShuttle.alt_sites.one_shortname = array[4];
					#valid_flag = 1;
					}
				}
			...
			if (ops == 3)
				...
				if ((value > 0) and (SpaceShuttle.alt_sites.entry_flag == 1))
					{
					var array = SpaceShuttle.set_alt_site_threshold(value);
					SpaceShuttle.landing_alt_site_two.set_latlon(array[0], array[1]);
					SpaceShuttle.alt_sites.two_index = array[2];
					SpaceShuttle.alt_sites.two_iata = array[3];
					SpaceShuttle.alt_sites.two_shortname = array[4];
					#valid_flag = 1;
					}
				}
...
if ((header == "SPEC") and (end =="PRO"))
	...
	if ((spec_num == 54) and (test_spec_ops_validity(spec54, major_mode) == 1))
		{
		page_select(idp_index, "p_dps_bearing");
		setprop("/fdm/jsbsim/systems/dps/spec", 54);		
		SpaceShuttle.idp_array[idp_index].set_spec(54);
		valid_flag = 1;
		}
...
if ((header == "ITEM") and (end = "EXEC"))
	...
	if (spec == 94)
		...
		if (item == 3)
			...
			if (int(value) > 0)
				{
				SpaceShuttle.pdrs_auto_seq_manager.assign_slot(0,int(value)-1);
				valid_flag = 1;
				}
			}
		else if (item == 14)
			...
			if (int(value) > 0)
				{
				SpaceShuttle.pdrs_auto_seq_manager.assign_slot(1,int(value)-1);
				valid_flag = 1;
				}
			}
		else if (item == 15)
			...
			if (int(value) > 0)
				{
				SpaceShuttle.pdrs_auto_seq_manager.assign_slot(2,int(value)-1);
				valid_flag = 1;
				}
			}
		else if (item == 16)
			...
			if (int(value) > 0)
				{
				SpaceShuttle.pdrs_auto_seq_manager.assign_slot(3,int(value)-1);
				valid_flag = 1;
				}
			}
		else if (item == 17)
...
if ((header == "SPEC") and (end =="PRO"))
	...
	if (major_function == 1) # GNC branch
		...
		if (spec_num == 18)
			...
			page_select(idp_index, "p_dps_fault");
			# calling the display with SPEC 99 PRO clears all fault messages
			SpaceShuttle.cws_message_array_long_bfs = ["","","","","","","","","","","","","","",""];
...
if ((header == "ITEM") and (end == "EXEC"))
	...
	if (((major_mode == 104)  or (major_mode == 301)) and (spec == 0))
		...
		#If guidance loaded, new PEG 7 entry will cancel it
		else if (item == 19)
			{
			#Illegal Entry while burn is on
			if (oms_ignited == 1) {valid_flag = 0;}
