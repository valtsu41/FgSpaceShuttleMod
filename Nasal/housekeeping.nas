...
# full OMS dump:  +14.1 lb propellant/sec
#########################################################################################
...
if (dump_mode == 1) {dump_rate = 14.1053 + 7.3505;}
...
# check a ttg requirement because oxidizer may run out before fuel / Dump ends after MECO during Contigency
#var manual_meco = SpaceShuttle.ssme_array[0].run_cmd + SpaceShuttle.ssme_array[1].run_cmd + SpaceShuttle.ssme_array[2].run_cmd;

if ((fuel_current < target_quantity) or (ttg < - 5) or (TAEM_guidance_TGINIT.IPHASE == 6))
#if ((fuel_current < target_quantity) or (ttg < - 5) or (manual_meco == 0))
	{
...
if (getprop("/fdm/jsbsim/systems/ap/orbital-dap-auto") != 1)
		{
		create_fault_message("    SEL AUTO  ","    ", "12   ", 3,"PASS");   #G2 fault
		}
...
var ev_timer =
{
	...
	obj.count_mode = "DOWN";
	obj.set_time = 0;
	obj.ref_time = 0;   # while running, the time at which 0 will be reached
	obj.power = 1;
    ...
    display : func
    {
	# We want the timer to count down to the next second only when actually reaching it, so round up.
	var display_time = math.ceil(me.time);

	if ((me.count_mode == "UP") and (me.count_flag == 1))
		{
		display_time = me.set_time - display_time;
		}

	var string =  SpaceShuttle.seconds_to_stringMS(display_time);
	if (display_time < 600) {string = "0"~string;}
	...
	me.ref_time = me.time_prop.getValue() + me.time;
	me.count_flag = 1;
	me.update();
    },
	...
	me.time = me.ref_time - me.time_prop.getValue();
	if (me.time < 0.0) {me.time = 0.0;}

	me.display();

	if (me.time == 0.0) {return;}

	# time - (ceil(time) - 1)  is the time until the next tick.
	# Add a tiny extra delay to make sure the timer does not run early.
	settimer(func me.update(), me.time - math.ceil(me.time) + 1.001);
    },
	...
	if (me.count_flag == 0) {return;}
	me.count_flag = 0;
	# Update one last time so that the final value of me.time is accurate
	me.time = me.ref_time - me.time_prop.getValue();
	me.display();
    },
	...
	if (me.count_flag) me.ref_time += inc;
    },
